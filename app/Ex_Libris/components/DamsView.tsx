...
export type DamsFilter = 'active' | 'retirado';
...
export default function DamsView({ filter }: { filter: DamsFilter }) {
  ...
  const { isRetired, retireDam, unretireDam, loading: retirementsLoading } = useDamRetirements();
  const dams = useMemo(
    () => allDams.filter(d => filter === 'retirado' ? isRetired(d.id) : !isRetired(d.id)),
    [allDams, isRetired, filter]
  );

  const title = filter === 'retirado' ? 'Dams Retiradas' : 'Existing Dams 2026';
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          {filter === 'active' ? (
                            ...
                            <button
                              ...
                              className="font-medium bg-white/5 hover:bg-green-500/20 hover:text-green-300 hover:border-green-500/40 text-gray-400 border border-white/15 px-2 py-0.5 rounded transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed">
                              ↺
                            </button>
                          )}
                        </div>
