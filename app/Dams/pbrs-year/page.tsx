...
export default function DamsPBRSYearPage() {
  const router = useRouter();
  const [searchQuery, setSearchQuery] = useState('');
  const [editingReport, setEditingReport] = useState<Report | null>(null);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);

  const { reports, loading, error, addReport, updateReport, deleteReport } = useReports('dams-pbrs-year');
  ...
  return (
    <div className="min-h-screen bg-[#0a192f] text-white p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Page Title */}
        <h1 className="text-4xl font-light mb-8">PBRS - Year</h1>
        ...
        {/* Add Report Button */}
        {!loading && <AddReportButton onAddReport={handleAddReport} location="dams-pbrs-year" />}
